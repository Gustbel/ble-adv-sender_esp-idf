...
static void ble_update(void *pvParameters)
{
	while(1)
	{
		...
		printf("Data[11] updated, value: 0x%x -- ", adv_data_raw[11]);

		status = esp_ble_gap_config_adv_data_raw((uint8_t*)&adv_data_raw, sizeof(adv_data_raw));
...
void ble_adv_init()
{
    ...
    if (status == ESP_OK){
        ...
        ESP_LOGE(DEMO_TAG, "Config BLE Adv sender failed: %s\n", esp_err_to_name(status));
    }
}
...
void app_main(void)
{
	    ...
	    ble_adv_init();
	    xTaskCreate(&ble_update, "update", 28192, NULL, 2, NULL);
}
